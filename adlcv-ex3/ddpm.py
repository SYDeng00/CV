...
class Diffusion:
    ...
    def get_betas(self, schedule='linear'):
        if schedule == 'linear':
            return torch.linspace(self.beta_start, self.beta_end, self.T).to(self.device) # HINT: use torch.linspace to create a linear schedule from beta_start to beta_end
