...
class Classifier(nn.Module):
    def __init__(self, img_size=16, c_in=3, labels=5, time_dim=256, device="cuda", channels=32):
        super().__init__()
        self.unet = UNet(img_size=img_size, c_in=c_in, c_out=labels, time_dim=time_dim, device=device, channels=channels, num_classes=labels)
        self.adaptive_pool = nn.AdaptiveAvgPool2d(1)

    def forward(self, x, t):
        logits = self.unet(x, t)
        logits = self.adaptive_pool(logits) # B x 5 x 1 x 1
        logits = torch.flatten(logits, 1) # B x 5 [batch_size, num_classes]
        return logits