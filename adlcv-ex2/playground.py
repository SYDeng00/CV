...
def image_to_patches(image, patch_size, image_grid=True):
    """
        image : torch.Tensor of shape B x C x H x W  -- B: batch size, C: number of channels, H: height, W: width
    ...
    if image_grid: # return in the same way as the input image -> grid of patches
        print(f'number of patches: {num_patches}')
        """
        Split H and W to patches
        HINT: B x c x H x W ------>>>> B x C x num_patches_h x patch_h x num_patches_w x patch_w
        where H = num_patches_h * patch_h, W=num_patches_w * patch_w to
        """
        patches = rearrange(image, 'b c (np_h p_h) (np_w p_w) -> b c np_h p_h np_w p_w',
                                    np_h=num_patches_h, p_h=patch_h, np_w=num_patches_w, p_w=patch_w)
        ...
        """
        patches = rearrange(patches, 'b c np_h p_h np_w p_w -> b (np_h np_w) c p_h p_w')
    else: # return as a sequence of flattened patches
        """
